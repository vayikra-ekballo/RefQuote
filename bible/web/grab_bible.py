...
def chunk_out(html: str, translation: str):
	...
	if translation == 'NIV':
		...
		if len(std_text_l) != 1:
			raise Exception("Expected a single <div> with class='result-text-style-normal'. Got %d." % len(std_text_l))
		std_text = std_text_l[0]
		...
		if footnotes is not None:
			...
			for comment in std_text.find_all(text=lambda text: isinstance(text, Comment)):
				comment.extract()
