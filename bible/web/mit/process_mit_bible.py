...
class BibleChapter(RawBibleChapter):
    def __init__(self, ch: RawBibleChapter, log=False):
        ...
        self.subheading, self.verses, self.skipped_verse_numbers = self.process_verses(
            portions["verses_html"]
        )
    ...
    def footnotes_html_to_array(html):
        ...
        for li in soup.find_all("li"):
            ...
            for (
                elem
            ) in li.contents:  # Use contents instead of descendants to avoid repetition
                if isinstance(elem, str):
                    ...
                    text += "".join(
                        f"*{sub_elem.get_text()}*" if sub_elem.name == "i" else sub_elem
